...
async def ask(request: VisionRagRequest, current_user: TokenPayload | None = Depends(get_current_user_optional)) -> VisionRagResponse:
    ...
    try:
        return await run_in_threadpool(pipeline.ask, question, request.imageDescription.strip(), customer_id)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
...
async def ask_with_image(
    ...
    if image is not None:
        ...
        extracted_signals = await run_in_threadpool(_extract_visual_signals, image_bytes, image.content_type)
    image_context = await run_in_threadpool(_compose_image_context, imageDescription, image)
    if extracted_signals:
        image_context = (image_context + "\n" if image_context else "") + f"Vision extraction: {extracted_signals}"
    consistency_label, consistency_score, consistency_reasons = await run_in_threadpool(
        _evaluate_consistency, imageDescription, extracted_signals,
    )
    try:
        base = await run_in_threadpool(pipeline.ask, cleaned_question, image_context, customer_id)
