...
class HierarchicalPipeline:
    ...
    def ask_stream(self, question: str, customer_id: str | None = None) -> Generator[str, None, None]:
        ...
        if not docs:
            ...
            yield sse_event("meta", {"chunksUsed": 0, "strategy": "chroma-direct+fallback-no-context", "orchestrationStrategy": "langchain-parent-child-retriever"})
        ...
        yield from stream_llm_response(
            self._llm, messages,
            metadata={"chunksUsed": len(docs), "strategy": "chroma-direct+parent-child-retriever", "orchestrationStrategy": "langchain-parent-child-retriever"},
        )
